#! /bin/bash

. ./config.sh

for ((i = 0; i < "${#nameOfLoci[@]}"; i++)); do

  echo "[INFO] Start to collect all files !"
  echo "------------------------------------Raw data------------------------------------" >"${resultDataPath}/qcReport.txt"
  seqkit stats "${ampliconInfo}${R1FastqGz}" >>"${resultDataPath}/qcReport.txt"
  seqkit stats "${ampliconInfo}${R2FastqGz}" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "-------------------------------Fastp quality trim-------------------------------" >>"${resultDataPath}/qcReport.txt"
  seqkit stats "${resultDataPath}trim_R1FastqGz.gz" >>"${resultDataPath}/qcReport.txt"
  seqkit stats "${resultDataPath}trim_R2FastqGz.gz" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "----------------------Cutadapt demultiplex by locus primer----------------------" >>"${resultDataPath}/qcReport.txt"
  seqkit stats "${resultDataPath}${nameOfLoci[i]}_result/${nameOfLoci[i]}_amplicon_r1.fq" >>"${resultDataPath}/qcReport.txt"
  seqkit stats "${resultDataPath}${nameOfLoci[i]}_result/${nameOfLoci[i]}_amplicon_r2.fq" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "---------------------Cutadapt demultiplex by sample barcode---------------------" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/demultiplexResult/untrimmed" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "-------------------------Cutadapt trim the primer sites-------------------------" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/demultiplexResult/trimmed" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "----------------------------------DADA2 filter----------------------------------" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/demultiplexResult/filtered" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "----------------------------------DADA2 denoise---------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "dada2 r1 denoise" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/denoiseResult/r1" >>"${resultDataPath}/qcReport.txt"
  echo "dada2 r2 denoise" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/denoiseResult/r2" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "-----------------------------------DADA2 merge----------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "dada2 r1 merge" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/mergeResult/dada2/merged" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------DADA2 10N concat--------------------------------" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/mergeResult/merger/nCatR1R2" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "----------------------------------Merger blast----------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "blast r1" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/mergeResult/merger/r1Ref" >>"${resultDataPath}/qcReport.txt"
  echo "blast r2" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/mergeResult/merger/r2Ref" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  #echo "----------------------------------Merger align----------------------------------" >> "${resultDataPath}/qcReport.txt"
  #echo "#TODO" >> "${resultDataPath}/qcReport.txt"
  #echo "--------------------------------------------------------------------------------" >> "${resultDataPath}/qcReport.txt"
  echo "----------------------------------Merger merge----------------------------------" >>"${resultDataPath}/qcReport.txt"
  ls "${resultDataPath}${nameOfLoci[i]}_result/mergeResult/merger/merged" >>"${resultDataPath}/qcReport.txt"
  echo "--------------------------------------------------------------------------------" >>"${resultDataPath}/qcReport.txt"
  echo "[INFO] End of collecting all files !"

done
