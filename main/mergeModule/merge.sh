...
if [ "${blastReadChoosingMode}" -eq 1 ]; then
  ...
  for ((i = 0; i < ${#nameOfLoci[@]}; i++)); do
    python3 "${workingDirectory}"/mergeModule/nnSpliter.py "$resultDataPath" "${nameOfLoci[i]}" #blast完，需要拆10N
  done
else #沒有的就跳過
  echo "[INFO] blastReadChoosingMode is 0, run nnSpliter.py after blast "
fi

bash "${workingDirectory}"/mergeModule/00_blastForRef.sh "$1" #先blast，內部自帶迴圈處理
...
for ((i = 0; i < ${#nameOfLoci[@]}; i++)); do
  ...
  if [ "${count}" -ge 1 ]; then #大於或等於1個檔案的才做
    ...
    python3 "${workingDirectory}"/mergeModule/blastResultParser.py "$ampliconInfo" "$resultDataPath" "${blastParsingMode[i]}" "${nameOfLoci[i]}"
    ...
    python3 "${workingDirectory}"/mergeModule/alignmentPretreater.py "$ampliconInfo" "${sseqidFileName[i]}" "$resultDataPath" "${nameOfLoci[i]}"
    python3 "${workingDirectory}"/mergeModule/alignmenter.py "$ampliconInfo" "$resultDataPath" "${nameOfLoci[i]}"
    python3 "${workingDirectory}"/mergeModule/merger.py "$ampliconInfo" "$resultDataPath" "${nameOfLoci[i]}"
