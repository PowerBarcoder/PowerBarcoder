...
for ((i = 0; i < "${#nameOfLoci[@]}"; i++)); do #迴圈處理，若dada2出來的nonmerge資料夾內為空，則須停止後續步驟
  ...
  if [ "${blastReadChoosingMode}" -eq 1 ]; then
    echo "[INFO] blastReadChoosingMode is 1, catQuery built from r1 and r2 "
      # # r1r2分開blast
  ...
  elif [ "${blastReadChoosingMode}" -eq 0 ]; then
    echo "[INFO] blastReadChoosingMode is 0, catQuery built from nCatR1R2 "
      # # r1r2 cat起來blast
