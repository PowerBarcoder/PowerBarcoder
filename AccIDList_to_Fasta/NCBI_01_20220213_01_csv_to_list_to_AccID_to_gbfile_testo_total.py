...
Load_file_dir=path+"\\"
Save_file_dir=path+"\\acclist\\"

# CSV to List
with open(Load_file_dir+'seqID_sequence.fasta_edit.csv',encoding='UTF-8') as f:
    reader = csv.reader(f)
    data = list(reader)
    # print(len(data[:-1]))
...
AccID=[]    
for i in data[:-1]:
    # 因為最後一行多一個/n，所以要減一，如果其他資料有刪好，就不到負一
    # print(i[0])
    AccID.append(i[0])
...
#這邊用字典把keyword變成每200個AccID一組(647個seq所以應該4組)
A_dict={}

for i in range(1,5):
    A_list_name="A"+str(i)+"_200"
    #print(A_list_name)
    A_list=AccID[(200*(i-1)):(200*i)]
    #print(A_list)
    A_dict.setdefault(A_list_name, A_list)
# print(A_dict.get("A0_200"))
# print(A_dict)
# 'A0_200' 'A1_200' 'A2_200' 'A3_200'
...
for i in range(2,5):
    try:
        text=A_dict.get("A%s_200"%i)
        time.perf_counter()
        #print(text)
        text_rep=str(text).replace("', '", " ")
        ...
        def RETRIVE():
            try:
                ...
                with open(Save_file_dir+x_rep+".gb","w",encoding='UTF-8') as file:
                    file.write(x)
                #print("RetriveA")
            except:
        ...
        if remainder!=0:
            while page!=1:
                num=1
                while num!=int(B)+1:#這裡需要修改，不然最後一頁會異常終止，雖然還是載得完
        ...
        print("A%s_200"%i+" finished "+"from "+ starttime + " to " + endtime)

        #os.rename("filename.txt","%s.gb"%i)#前面已經用gb當檔名了
        END()
    except  Exception as e:
        print("A%s_200"%i+" is failed")
        print(e)
        pass
